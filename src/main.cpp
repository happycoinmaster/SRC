...
uint256 hashGenesisBlock("0x5445eaeb178e81fe144327f589779139aa2d5e2370a5876227982cadc92b7967");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Zilbercoin: starting difficulty is 1 / 2^12
...
{
    // Zilbercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 112 * COIN;

            if(nHeight == 1)
            {
            nSubsidy = 8301176 * COIN;
            }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Zilbercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Zilbercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1485294910, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "The Times 25.01.2017 Chancellor on brink of second bailout for banks";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 112 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0428a507e3b54185211409b87f169cabaed942c1ea93af497a124a535628b356c3296515584bf0684d5a75e74f76a969840dbbd77200e7c1fa57957bd8cd594fc4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1485294910;
        ...
        assert(block.hashMerkleRoot == uint256("0x77c1f9c6d1733a3f93d1bdb2692cf75d24090aa526b7afb9b1d5693fc517e5fd"));
...
unsigned char pchMessageStart[4] = { 0xef, 0xd9, 0x74, 0x20 }; // Zilbercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ZilbercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ZilbercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ZilbercoinMiner(CWallet *pwallet)
{
    printf("ZilbercoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ZilbercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ZilbercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ZilbercoinMiner, pwallet));
}
